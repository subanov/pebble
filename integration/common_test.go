...
func makeClient(contacts ...string) (*client, error) {
	c, err := acme.NewClient(os.Getenv("DIRECTORY"), acme.WithInsecureSkipVerify())
	if err != nil {
...
func addHTTP01Response(token, keyAuthorization string) error {
	resp, err := http.Post("http://localhost:8055/add-http01", "",
...
func delHTTP01Response(token string) error {
	resp, err := http.Post("http://localhost:8055/del-http01", "",
		bytes.NewBufferString(fmt.Sprintf(`{
		"token": "%s"
	}`, token)))
	if err != nil {
